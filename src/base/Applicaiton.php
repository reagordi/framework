...
class Applicaiton
{
  ...
  private function dbConfig()
  {
    ...
    if ($db === null) {
      ...
      } elseif (is_file(APP_DIR . '/php_interface/db.php')) $db =
        require_once APP_DIR . '/php_interface/db.php';
      else {
        $db_config_err = true;
      }
    }
    ...
    if ($db_config_err) {
      if (PHP_SAPI !== 'cli') {
        if (is_dir(ROOT_DIR . '/install/')) {
          $server = \Reagordi\Framework\Context::getCurrent()->getServer();
          $path = '';
          if (\Reagordi\Framework\Context::getCurrent()->getRequest()->isHttps()) {
            $path .= 'https://';
          } else $path .= 'http://';
      ...
      die('MediaLife Framework is not installed. To install, go to /install/');
    }
    unset($db_config_err);
    $this->config_db = $db;
  ...
  public function dbInit()
  {
    ...
    $uuid_writer = new \SDesya74\Tools\UUIDWriterMySQL( $old_adapter );
